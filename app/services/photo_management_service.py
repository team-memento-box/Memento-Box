...
class PhotoManagementService:
    def __init__(self, db: AsyncSession):
        self.db = db
        self.blob_connection_string = settings.AZURE_STORAGE_CONNECTION_STRING
        self.container_name = settings.AZURE_STORAGE_CONTAINER_NAME or "photos"
    ...
    async def create_photo(
        ...
        try:
            ...
            # 파일 저장 (로컬)
            image_path, image_url = await self._save_file_locally(file_data, file.filename)
